...
z, F = emus.calculate_zs(psis, n_iter=2)

z_avars = iter_avar.calc_log_z(psis, z, repexchange=False)
...
log_zerr_iter, log_zcontribs_iter, log_ztaus_iter = iter_avar.calc_log_z(
    ...
    psis, z_iter, iat_method=1, repexchange=True)
print(log_zerr_iter)

N = len(z)
plt.errorbar(np.arange(N), -np.log(z), yerr=np.sqrt(log_zerr), label='noniterative')
plt.errorbar(np.arange(N), -np.log(z_iter), yerr=np.sqrt(log_zerr_iter), label='iterative')
plt.legend()
plt.show()

plt.plot(np.arange(N), log_zerr, label='noniterative')
plt.plot(np.arange(N), np.sqrt(log_zerr_iter), label='iterative')
plt.legend()
plt.show()


# # Calculate error in each z value from the first iteration.
# iter_avar.calc_log_z(
#     psis, z, iat_method='acor', repexchange=True)
# raise Exception()
