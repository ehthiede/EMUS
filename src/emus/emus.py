...
def calculate_zs(psis, neighbors=None, n_iter=0, tol=DEFAULT_ITER_TOL, use_iats=False, iat_method=DEFAULT_IAT, kappa=None):
    ...
    if use_iats:
        z, F, iats = emus_iter(psis, neighbors=neighbors,
                               return_iats=use_iats, iat_method=iat_method)
    else:
        z, F = emus_iter(psis, neighbors=neighbors,
                         return_iats=use_iats, iat_method=iat_method)
...
def emus_iter(psis, Avals=None, neighbors=None, return_iats=False, iat_method=DEFAULT_IAT, kappa=None):
    ...
    for i in range(L):
        ...
        Fi_out = calculate_Fi(psis[i], nbr_index,
                              A_nbs, return_iats, kappa_nbs)
        if return_iats:
            Fi, trajs = Fi_out
    ...
    z = lm.stationary_distrib(F, verbose=False)
    if return_iats:
        return z, F, iats
    else:
        return z, F
