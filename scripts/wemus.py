...
import argparse
import os
from emus._defaults import DEFAULT_K_B, DEFAULT_T
import h5py
...
def main():
    ...
    psis, cv_trajs, neighbors = uu.data_from_meta(
        a['meta_file'], a['n_dim'], T=a['T'], k_B=a['k_B'], period=a['period'], nsig=a['sigma'])
    ...
    pmf, edges = emus.calculate_pmf(
        cv_trajs, psis, domain, z, neighbors=neighbors, nbins=a['nbins'], kT=kT)

    # Calculate any averages of functions.
    if fdata is not None:
        favgs = []
        for n, fdata_i in enumerate(fdata):
            favgs.append(emus.calculate_obs(
                psis, z, fdata_i, neighbors=neighbors))
    ...
    if a['error'] is not None:
        ...
        pmf_EMUS, pmf_EMUS_avar = avar.calc_pmf(
            cv_trajs, psis, domain, zEMUS, FEMUS, neighbors=neighbors, nbins=a['nbins'], kT=kT, iat_method=z_avg_taus)
        ...
        if fdata is not None:
            ...
            for fdata_i in fdata:
                iat, mean, variances = avar.calc_avg_ratio(
                    psis, zEMUS, FEMUS, fdata_i, neighbors=neighbors, iat_method=a['error'])
    ...
    dmn_dset = pmf_grp.create_dataset(
        "domain", np.array(domain).shape, dtype='f')
    pmf_dset[...] = pmf
    ...
    for ie, edg in enumerate(edges):
        ...
        edg_dset[...] = edg
    if a['error'] is not None:
        pmf_EMUS_dset = pmf_grp.create_dataset(
            "pmf_no_iter", pmf_EMUS.shape, dtype='f')
        pmf_EMUS_dset[...] = pmf_EMUS
        pmf_EMUS_avar_dset = pmf_grp.create_dataset(
            "pmf_no_iter_avars", pmf_EMUS_avar.shape, dtype='f')
    ...
    F_dset[...] = F
    if a['error'] is not None:
        zerr_dset = z_grp.create_dataset(
            "z_avars", np.array(zvars).shape, dtype='f')
    ...
    if fdata is not None:
        ...
        if a['error'] is not None:
            fvar_dset = f_grp.create_dataset(
                "f_avars", np.shape(ferrs), dtype='f')
            fvar_dset[...] = ferrs
    f.close()
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Performs EMUS analysis on sampled data stored in a wham-like format.")
    parser.add_argument('n_dim', type=int,
                        help='The number of collective variables of the data')
    parser.add_argument('metafile', type=str,
                        help='Path to the wham-like meta file')
    ...
    parser.add_argument('-f', '--fxnfile', type=str,
                        help="Path to meta file containing paths to observable information.")
    parser.add_argument('-T', '--temperature', type=float, default=DEFAULT_T,
                        help='Temperature of the data, default is %.4f.  Any temperature that is provided in the meta file overrides this.' % DEFAULT_T)
    parser.add_argument('-k', '--boltz', type=str,
                        help="Units for the Boltzmann constant. Default is k_B=%.4f, other choices include 'kCal' for kCal/mol and 'kJ', or a numerical value of the Boltzmann constant." % DEFAULT_K_B)
    parser.add_argument('-n', '--n_iter', type=int, default=0,
                        help="Number of iterative EMUS iterations to perform on the data set.  Note that error analysis is only for the initial iteration.")
    parser.add_argument('-s', '--sigma', type=float, default=6,
                        help="Do not evaluate F_{ij} if window centers are more than this number of standard deviations apart.  Default is 6.")
    ...
    parser.add_argument(
        '-o', '--output', help="Base string for data to output.  Data is saved to [output].hdf5.  Default is the name of the meta file.")
    ...
    if (args.boltz == 'kCal') or (args.boltz == 'kcal'):
        ...
        try:
            ...
            raise ValueError(
                "Unable to convert input for k_B to a float.  Input string was %s" % args.boltz)
    newargs['k_B'] = k_B
        ...
        for pndx, p in enumerate(args.period):
            if ((p == '0') or (p == '0.0')):
                ...
                try:
                    ...
                    raise ValueError(
                        "Unable to convert input for period in dimension %d to a float.  Input string was %s" % (pndx, args.boltz))
                period.append(pd)
        if len(period) != n_dim:
            raise ValueError("Periodicity information has length %d but %d dimensions were specified" % (
                len(period), n_dim))
    ...
    if len(args.pmf) != 3*n_dim:
        raise ValueError("Mismatch between the no. of parameters provided for the PMF and the number of dimensions.  For %d dimensions %d parameters needed, but only %d were found.  PMF parameter input is " % (
            n_dim, 3*n_dim, len(args.pmf)) + str(args.pmf))
    for nd in range(n_dim):
        ...
        if args.pmf[ndx] == 'pi':
            ...
            try:
                ...
                raise ValueError(
                    "Unable to parse first argument for PMF parameters in dimension %d." % nd)
        ...
        if args.pmf[ndx+1] == 'pi':
            ...
            try:
                ...
                raise ValueError(
                    "Unable to parse second argument for PMF parameters in dimension %d." % nd)
        # Number of Histogram Bins
        try:
            nbins.append(int(args.pmf[ndx+2]))
        except ValueError:
            raise ValueError(
                "Unable to parse third argument for PMF parameters in dimension %d." % nd)
