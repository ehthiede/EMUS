...
def _parse_args():
    ...
    parser.add_argument('ndim',type=int,help='The number of collective variables of the data') 
    parser.add_argument('metafile',type=str,help='Path to the wham-like meta file')
    parser.add_argument('pmf',type=str, nargs='+',help="Information on the domain of the PMF.  Should be a list of 3*ndim values, where each triple is the lowest value in that dimension, the highest value, and the number of histogram bins in that dimension respectively.  The parser will also interpret 'pi' for 3.14..., or 'npi' for negative pi.")
    parser.add_argument('-p','--period',type=str, nargs='+',help="Optional periodicity information for the collective variables.  Format is a list of ndim values, where each float is either the period of the variable, 'pi' if the value has period of 2 pi (e.g. units are radians), or 0 if the collective variable is aperiodic.")
    parser.add_argument('-f','--fxnfile',type=str,help="Path to meta file containing paths to observable information.")
    parser.add_argument('-T','--temperature',type=float,default=DEFAULT_T,help='Temperature of the data, default is %.4f.  Any temperature is provided in the meta file overrides this.'% DEFAULT_T)
    ...
    parser.add_argument('-n','--nMBAR',type=int,default=0,help="Number of MBAR Iterations to perform on the data set.  Note that error analysis is only for the initial iteration.")
    ...
    parser.add_argument('--ext',type=str,choices=['hdf5', 'txt'],default=DEFAULT_EXT,help=r"File extension for output. Default is %s; supported options are 'hdf5' and 'txt' (text output only available ndim < 3, hdf5 requires h5py to be installed).  Note that hdf5 saves everything to one large file, whereas .txt saves to multiple named files." % DEFAULT_EXT)
...
#    parser.add_argument('-v','--verbose',typestr,help="Path to meta file containing paths to observable information.")
    ...
    newargs['ndim'] = args.ndim
    ndim = args.ndim
    newargs['nMBAR'] = args.nMBAR
    newargs['error'] = args.error
    newargs['ext'] = args.ext
    newargs['sigma'] = args.sigma
    if (newargs['ext'] == 'txt') and (ndim > 2):
    ...
    if args.period is None:
        ...
        for pndx,p in enumerate(args.period):
            if (p == '0') or (p == '0.0'):
                ...
                period.append(pd)
        if len(period) != ndim:
            raise ValueError("Periodicity information has length %d but %d dimensions were specified"%(len(period),ndim))
    newargs['period'] = period

    # PMF domain information
    domain = []
    nbins = []
    if len(args.pmf) != 3*ndim:
        raise ValueError("Mismatch between the no. of parameters provided for the PMF and the number of dimensions.  For %d dimensions %d parameters needed, but only %d were found.  PMF parameter input is "%(ndim,3*ndim,len(args.pmf)) + str(args.pmf))
    for nd in xrange(ndim):
